...
from models.model import get_timesformer_model
...
def main():
    ...
    img_size = config['train_transforms']['resize'][0]
    ...
    for epoch in range(num_epochs):
        ...
        for clips, labels in train_loader:
            # clips: (batch_size, T, C, H, W)
            # labels: (batch_size, T) または (batch_size,)
            clips = clips.to(device)
            labels = labels[:, clip_length // 2]  # 中央のフレームのラベルを使用
            labels = labels.to(device)

            optimizer.zero_grad()
            outputs = model(clips)  # (batch_size, num_classes)
